...
def map_data():
    ...
    rearrange_cols_df = rearrange_cols_df.groupby(['Geography', 'Year','Type']).sum().reset_index()

    rearrange_cols_df.rename(columns={'Geography': 'City'}, inplace=True)
    # Rename columns
    rearrange_cols_df[['s_city', 's_city_2']] = rearrange_cols_df.City.str.split(pat=chr(47), expand=True)
    ...
    city_total = data_complete_df.to_dict('records')
